...
class PaytmController extends Controller
{
    ...
    public function paymentCallback()
    {
        ...
        if ($transaction->isSuccessful()) {
            ...
            return redirect('initiate.payment')->with('message', "Your payment is successfull.");
        } else if ($transaction->isFailed()) {
            Paytm::where('order_id', $order_id)->update(['status' => 0, 'transaction_id' => $transaction->getTransactionId()]);
            return  redirect('initiate.payment')->with('message', "Your payment is failed.");
            
        } else if ($transaction->isOpen()) {
            Paytm::where('order_id', $order_id)->update(['status' => 2, 'transaction_id' => $transaction->getTransactionId()]);
            return  redirect('initiate.payment')->with('message', "Your payment is processing.");
        }
